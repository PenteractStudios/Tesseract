...
namespace ImGui {
	...
	int Curve(const char* label, const ImVec2& size, const int maxpoints, ImVec2* points) {
		...
		if (hovered) {
			...
			if (g.IO.MouseDown[0]) {
				modified = 1;
				ImVec2 pos = ((float2) g.IO.MousePos - bb.Min).Div((float2) bb.Max - bb.Min);
				pos.y = 1 - pos.y;
				...
				ImVec2 p = (float2) points[left] - pos;
				float p1d = sqrt(p.x * p.x + p.y * p.y);
				p = (float2) points[left + 1] - pos;
		...
		// bg grid
		window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y + ht / 2), ImVec2(bb.Max.x, bb.Min.y + ht / 2), GetColorU32(ImGuiCol_TextDisabled), 3);

		window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y + ht / 4), ImVec2(bb.Max.x, bb.Min.y + ht / 4), GetColorU32(ImGuiCol_TextDisabled));

		window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y + ht / 4 * 3), ImVec2(bb.Max.x, bb.Min.y + ht / 4 * 3), GetColorU32(ImGuiCol_TextDisabled));

		for (i = 0; i < 9; i++) {
			window->DrawList->AddLine(ImVec2(bb.Min.x + (wd / 10) * (i + 1), bb.Min.y), ImVec2(bb.Min.x + (wd / 10) * (i + 1), bb.Max.y), GetColorU32(ImGuiCol_TextDisabled));
		}
		...
		for (i = 1; i < max; i++) {
			...
			a.y = 1 - a.y;
			b.y = 1 - b.y;
			a = a.Mul((float2) bb.Max - bb.Min) + bb.Min;
			b = b.Mul((float2) bb.Max - bb.Min) + bb.Min;
		...
		if (hovered) {
			...
			for (i = 0; i < max; i++) {
				float2 p = points[i];
				p.y = 1 - p.y;
				p = p.Mul((float2) bb.Max - bb.Min) + bb.Min;
		...
		ImGui::TextColored(ImVec4(0.35f, 0.69f, 0.87f, 1.0f), "Generate Curve:");
    ...
    const char* items[] = {"Custom",
                           ...
                           "Expo in  out",    "Circ in",     "Circ out",    "Circ in  out",  "Bounce in",  "Bounce out",
                           "Bounce in out",   "Sine square", "Exponential", "Schubring1",    "Schubring2", "Schubring3",
                           "SinPi2",          "Swing"};
		// clang-format on
		static int item = 0;
		if (modified) {
		...
		if (ImGui::Combo("Type", &item, items, IM_ARRAYSIZE(items))) {
			max = maxpoints;
			if (item > 0) {
				for (i = 0; i < max; ++i) {
